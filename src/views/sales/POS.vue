<template>
  <div class="min-h-screen bg-gray-100 flex flex-col">
    <div class="flex h-screen">
      ...
      <div class="w-96 bg-white border-l border-gray-200 flex flex-col shadow-lg overflow-hidden">
        <!-- Cart Header -->
        <div class="bg-gradient-to-r from-primary-500 to-primary-600 text-white px-4 py-3 border-b flex-shrink-0">
          <h2 class="text-base font-bold">🛒 Shopping Cart</h2>
          <p class="text-xs text-primary-100">{{ cartItems.length }} item(s)</p>
        </div>
        ...
        <div class="flex-1 overflow-y-auto min-h-0 bg-white">
          <div v-if="cartItems.length > 0" class="divide-y divide-gray-200">
            <div
              ...
              :key="idx"
              class="p-3 hover:bg-gray-50 transition-colors border-b"
            >
              <!-- Item Header -->
              <div class="flex items-start justify-between mb-2">
                <div class="flex-1">
                  <h4 class="font-semibold text-gray-900 text-xs">{{ item.product_name }}</h4>
              ...
              <div class="flex items-center gap-1 mb-2">
                <button
                  ...
                  class="px-2 py-0.5 bg-gray-200 hover:bg-gray-300 rounded text-xs font-bold"
                >
                  −
                </button>
                <input
                  ...
                  type="number"
                  class="w-10 text-center px-1 py-0.5 border border-gray-300 rounded text-xs"
                />
                <button
              ...
              <div class="flex justify-between items-center">
                ...
                <p class="text-xs font-bold text-gray-900">₱{{ formatPrice(item.quantity * item.selling_price) }}</p>
              </div>
            </div>
          </div>
          ...
          <div v-else class="flex flex-col items-center justify-center h-48">
            <p class="text-gray-400 text-2xl">🛍️</p>
            <p class="text-gray-500 text-xs mt-2">Cart is empty</p>
            <p class="text-xs text-gray-400">Select products to add</p>
          </div>
        </div>

        <!-- Cart Summary & Payment -->
        <div class="border-t bg-gray-50 p-3 space-y-2 flex-shrink-0 overflow-y-auto max-h-96">
          <!-- Subtotal -->
          ...
          <!-- Total -->
          <div class="flex justify-between bg-primary-50 p-2 rounded border border-primary-200">
            <span class="font-bold text-gray-900 text-sm">TOTAL:</span>
            <span class="font-bold text-primary-600 text-sm">₱{{ formatPrice(totalAmount) }}</span>
          </div>
          ...
          <div class="space-y-1">
            ...
            <input
              ...
              type="number"
              placeholder="0.00"
              class="w-full px-2 py-1 border border-gray-300 rounded text-lg font-bold focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
          </div>
          ...
          <div class="bg-green-50 p-2 rounded border border-green-200">
            ...
            <p class="text-lg font-bold text-green-600">₱{{ formatPrice(Math.max(0, amountPaid - totalAmount)) }}</p>
          </div>
          ...
          <button
            ...
            :class="[
              'w-full py-2 rounded font-bold text-sm transition-all duration-200 mt-2',
          ...
          <button
            v-if="cartItems.length > 0"
            @click="printReceipt"
            class="w-full py-1 rounded font-medium text-xs text-gray-700 bg-gray-200 hover:bg-gray-300 transition-colors mt-1"
          >
            🖨️ Print
          </button>
        </div>
